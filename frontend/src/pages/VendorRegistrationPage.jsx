...
const BLOCKED_EMAIL_DOMAINS = ['gmail.com', 'yahoo.com'];
...
export default function VendorRegistrationPage() {
  ...
  const getDuplicateContactIndexes = (key) => {
    const valueMap = form.contacts.reduce((acc, contact, index) => {
      const value = contact[key].trim().toLowerCase();
      if (!value) return acc;
      acc[value] = acc[value] || [];
  ...
  const validateForm = () => {
    ...
    if (!form.email.trim()) {
      ...
      nextErrors.email = 'Gmail and Yahoo email addresses are not allowed';
    }
    ...
    form.contacts.forEach((contact, index) => {
      ...
      } else if (isBlockedEmailDomain(contact.email)) {
        nextErrors.contacts[index].email = 'Gmail and Yahoo email addresses are not allowed';
      }
      if (!contact.phone.trim()) nextErrors.contacts[index].phone = 'Phone is required';
    });

    const duplicateEmailIndexes = getDuplicateContactIndexes('email');
    duplicateEmailIndexes.forEach((index) => {
      nextErrors.contacts[index].email = 'Contact person emails must be unique';
    });

    const duplicatePhoneIndexes = getDuplicateContactIndexes('phone');
    duplicatePhoneIndexes.forEach((index) => {
      nextErrors.contacts[index].phone = 'Contact person mobile numbers must be unique';
    });
              ...
              <Grid item xs={12} md={4} order={{ xs: 2, md: 1 }}>
                <Stack spacing={2} sx={{ position: { md: 'sticky' }, top: { md: 24 } }}>
                  <Box
                    ...
                    src={logo}
                    alt="Pawfect Foods"
                    sx={{ width: { xs: 140, sm: 160 }, maxWidth: '100%' }}
                  />
                  <Typography
                    variant="h4"
                    sx={{ fontWeight: 800, color: '#155c37', fontSize: { xs: '2rem', sm: '2.25rem' } }}
                  >
                    Vendor Registration
                  </Typography>
                  ...
                  <Box
                    sx={{
                      p: 1.5,
                      borderRadius: 2,
                      border: '1px dashed rgba(21, 92, 55, 0.28)',
                      backgroundColor: 'rgba(255, 255, 255, 0.58)',
                    }}
                  >
                    <Typography variant="body2" sx={{ color: '#24523a', fontWeight: 600 }}>
                      Requirements: use a unique business email, do not use Gmail or Yahoo, and keep each contact mobile number unique.
                    </Typography>
                  </Box>
                  <Box
                    sx={{
              ...
              <Grid item xs={12} md={8} order={{ xs: 1, md: 2 }}>
                <Stack component="form" onSubmit={submit} spacing={2.25}>
                  ...
                  <Stack direction={{ xs: 'column', md: 'row' }} spacing={2}>
                    ...
                    />
                    <TextField
                      label="Business Email (Gmail/Yahoo excluded)"
                      type="email"
                      value={form.email}
                      onChange={(e) => setFieldValue('email', e.target.value)}
                      error={Boolean(fieldErrors.email)}
                      helperText={fieldErrors.email || 'Use a unique business email. Gmail and Yahoo addresses are excluded.'}
                      fullWidth
                      required
                    />
                  </Stack>
                  ...
                  <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} alignItems={{ md: 'flex-start' }}>
                    ...
                    <Button
                      ...
                      disabled={gstLoading}
                      variant="outlined"
                      sx={{ minWidth: { md: 190 }, width: { xs: '100%', md: 'auto' }, height: 56 }}
                    >
                  ...
                  <Divider sx={{ borderColor: 'rgba(28, 110, 63, 0.15)' }} />
                  ...
                  {form.contacts.map((contact, index) => (
                    ...
                    >
                      <Typography variant="subtitle1" sx={{ fontWeight: 700, color: '#1d6c3e' }}>
                      ...
                      <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5}>
                        <TextField
                          label="Email (Gmail/Yahoo excluded)"
                          type="email"
                          value={contact.email}
                          onChange={(e) => setContact(index, 'email', e.target.value)}
                          error={Boolean(fieldErrors.contacts[index]?.email)}
                          helperText={fieldErrors.contacts[index]?.email || 'Use a unique business email for each contact person.'}
                          fullWidth
                          required
                        />
                        <TextField
                          ...
                          error={Boolean(fieldErrors.contacts[index]?.phone)}
                            helperText={fieldErrors.contacts[index]?.phone || 'Each mobile number must be unique.'}
                          fullWidth
                          required
                        />
                      </Stack>
